	...
	$(document).on('click', '#btEnviarPedido', function (e) {
		...
		verificarConexaoInternet().then(status => {
			if (status === 'conectado') {
						...
						if (obj.semEstoque) {
							...
							if (obj.rastreamento) {
								if (obj.pedido) {
									...
									$('#pedido .containerFinalizar #obrigado h2').html("Pedido " + obj.pedido + " concluí­do com sucesso!");
								...
								if (obj.telefone) {
									...
									meuPedido = meuPedido.replaceAll('Taxa de cartÃ£o:', '*Taxa de cartão:*');
				...
				$("#modal button").addClass('confirmar');
				mostrarPopup('Sem ConexÃ£o com a Internet',
					'Parece que vocÃª está offline. Verifique sua conexÃ£o com a internet e tente novamente.');
				$('#modalCarregando').hide();
				return
			}
		});
